...
- If k = total number of examples, then, the k-fold becomes leave-one-out cross validation, as only one example place in the validation set in each validation run.
- The stratified k-fold cross validation is suitable for instances where there is inbalance in the frequency of the classes. If we use a random sampling, as is the case in k-fold, some examples might not have enough contribution to some folds.
...
Reproducibility ensures that we can recreate experiments. Deep learning models has been notorious known to have many parameters and hyperparameters, with randomness in intializations and sampling. For reproducibility sake, we have to put things in order to validatate our experiments. Two main places to ensure this are; i) hyperparameter initializations and ii) random seed settings

### Structuring Hyperparameters
Firstly, we will define our hyperparameters and other setting in a structured way, so that hyperparameter changes can only be applied only at one single point. We can use a dictionary for this purpose or the argparse library. The argparge library is more prefered as it is an argument parsing tool which helps to easily translate from hyperparameters in notebooks to arguments on the command line. For an indepth tutorial on this check out this [argparse tutorial](https://towardsdatascience.com/learn-enough-python-to-be-useful-argparse-e482e1764e05) by Jeff Hale on Medium.
...
```
Then, we can easily call the function to setup the seeding across all libraries.
```
...
- the model of each fold can be saved to make predictions on an unseen data. Then, softmax predictions of examples of each fold can be averaged, select the correct class. This is popularly known as model ensembling.  

I will be using the [Cassava Disease Challenge](https://www.kaggle.com/c/ammi-2020-convnets) on Kaggle as a runnung example to explain the underlying concept. The challenge entailed classifying pictures of cassava leaves into 1 of 4 disease categories or healthy.   

I will only highlight parts of the code that perfroms the cross-validation and will not try to show all codes here. For the full code, you can check the [cassava disease classification](https://www.kaggle.com/ogunlao/crossvalidation-for-cassava-disease-classification) Kaggle kernel I created for this tutorial.
...
- The dataloaders then create batch of data for training and validation.  
...
This is pretty straight forward. Just redownload the weights or reload the weight from the stored cache at the begining of each fold.
2. Using a custom model built from scratch.
Just reinitialize your model on each fold. Since, we have set a seed, we are more likely to have the same set of weights and biases during reinitialization. You can also intialize once, store the weights in a cache and retrieve the stored weights for each fold.
...
> In any case, do not use the trained model of a previous fold to initialize a new fold. This outcome can be very*5 disastrous and deceptive!!
